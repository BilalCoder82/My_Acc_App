"""
tools/backfill_seed_je_pur_sequence.py
==========================================
حسم namespace "JE-PUR" (Purchase Invoice Posting) — نفس فئة مشكلة JV/JV-REV.
_next_ref_no() القديمة بـposting.py كانت تولّد "JE-PUR-{count+1:05d}"
(5 أرقام). journal_number_sequences يولّد الآن "JE-PUR-{last_value:06d}"
...
الاستخدام: python3 tools/backfill_seed_je_pur_sequence.py <db_path>
"""
...
NAMESPACE = "JE-PUR"
VALID_PATTERN = re.compile(r"^JE-PUR-(\d+)$")


def inspect_and_seed(db_path: str, apply: bool = True) -> dict:
    conn = sqlite3.connect(db_path, timeout=30)
    try:
        rows = conn.execute("SELECT id, ref_no FROM journal_entries WHERE ref_no LIKE 'JE-PUR-%'").fetchall()
        ...
        report = {
            "db_path": db_path,
            "valid_je_pur_refs_count": len(valid_numbers),
            "anomalies": anomalies,
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("الاستخدام: python3 tools/backfill_seed_je_pur_sequence.py <db_path>")
        sys.exit(1)
    r = inspect_and_seed(sys.argv[1])
    print(f"قاعدة البيانات: {r['db_path']}")
    print(f"مراجع JE-PUR صالحة: {r['valid_je_pur_refs_count']}")
