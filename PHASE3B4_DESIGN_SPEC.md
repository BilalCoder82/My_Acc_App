...
- **الزيادة الذرّية تتم بـTransaction مستقلة عن Transaction المستند — قرار تصميمي ملزم، وليس تفصيلاً تنفيذياً مؤجَّلاً**: عملية واحدة (`UPDATE journal_number_sequences SET last_value = last_value + 1 WHERE namespace = ? RETURNING last_value` أو ما يعادلها حسب دعم SQLite للإصدار المُستهدَف؛ البديل الآمن عالمياً: `UPDATE ... SET last_value = last_value + 1 WHERE namespace=?` ثم `SELECT last_value`) تُنفَّذ وتُثبَّت (`COMMIT`) **قبل** بدء أي عمل على `JournalEntry`/`JournalLine` بنفس الطلب — **لا يجوز** أن يقع هذا الـUPDATE ضمن نفس الـtransaction التي يمكن أن يُطبَّق عليها `rollback` لصالح المستند، وإلا فإن أي `rollback` للمستند يُعيد أيضاً قيمة `last_value` (سلوك SQLite الطبيعي لأي rollback على transaction يحوي كل تعديلاته)، فيُعاد إصدار نفس الرقم لاحقاً — وهو تناقض مباشر مع "لا إعادة استخدام" بـ§4. التفصيل التنفيذي الدقيق لـ**كيفية** عزل هذه الـtransaction (اتصال/Session مستقل، أم commit مبكر على نفس الـSession) يُحسَم عند الكتابة الفعلية (راجع التحذير أسفل §4)، لكن **مبدأ الاستقلالية نفسه غير قابل للتأجيل**.
...
- `begin_entry()` (ونظيرتها `post_immediate()` بالنمط ب) **يحجزان `ref_no` فوراً** كخطوة داخلية أولى (قبل أي شيء آخر بالدالة) — استدعاء واحد للـSequence بالـnamespace المناسب.
- **تسلسل إلزامي، غير قابل لأي عكس أو دمج**:
  1. حجز الرقم بـSequence Transaction مستقلة (§1) → `COMMIT` فوري لهذه الخطوة وحدها.
  2. **فقط بعد نجاح الخطوة 1**: بناء/تعديل `JournalEntry`/`JournalLine` ضمن transaction المستند العادية.
  3. نجاح transaction المستند → `COMMIT`. فشلها → `ROLLBACK` **يطال المستند فقط**، لا الرقم المحجوز أصلاً (لأنه ثُبِّت بخطوة منفصلة سابقة له، لا علاقة بنيوية تربطهما).
- **لا استرجاع (Rollback) للرقم المحجوز أبداً بعد ثباته بالخطوة 1** — لو فشل إنشاء/إكمال المستند بعدها بأي سبب، فالرقم **يُفقَد فعلياً كفجوة** (السلوك المقبول صراحة بالقرار المعماري) — **لا آلية "تحرير رقم" أو "إعادة استخدام" تُبنى إطلاقاً**، بما يطابق "لا نعيد استخدام الأرقام المحجوزة" حرفياً. هذا التصرف يتحقق **فقط** إذا التزم التنفيذ الفعلي بالتسلسل أعلاه بالضبط — أي دمج للخطوتين 1 و2 بنفس الـtransaction (كما كان النص السابق لهذا القسم يسمح ضمنياً) يُبطل الضمان بالكامل، لأن rollback المستند سيُعيد حينها قيمة الـSequence أيضاً.
- **تحذير تنفيذي يجب أن يبقى ماثلاً عند الكتابة الفعلية (بلا حسم هنا)**: إن استُخدمت نفس الـSQLAlchemy `Session` لكلا الخطوتين، فـ`commit()` على الخطوة 1 سيُثبِّت **أي** تغييرات أخرى معلَّقة (pending/flushed) على نفس الـSession في تلك اللحظة، ليس فقط تحديث الـSequence — وهذا خطر حقيقي إن استُدعي `begin_entry()`/`post_immediate()` من داخل تسلسل عمل أعرض له كتابات معلَّقة أخرى سابقة (كما بـ`settlements.py`/`invoice_cancel.py` المحتملين لاحقاً). الخيار الأكثر أماناً بنيوياً هو اتصال/Session **منفصل تماماً** مخصَّص فقط لجدول `journal_number_sequences`، لا commit مبكر على الـSession المشترك — لكن الاختيار النهائي بين الخيارين، وكيفية تفادي "database is locked" (SQLite تسمح بكاتب واحد فعّال بأي لحظة) عند الاتصال المزدوج، تفصيل تنفيذي يُحسَم عند الكتابة الفعلية، لا يُلزَم هنا.
...
جدول الـSequence مبني ومُختبَر (Migration + Backfill إن احتيج ترحيل أرقام موجودة لأول مرة لجدول العدّاد — تفصيل يُحسَم عند التنفيذ الفعلي بناءً على القيمة الحالية الفعلية لآخر رقم بكل namespace، **مع قراءة فعلية لكل `ref_no` الموجودة فعلاً وتحديد namespaces وقيمها القصوى منها، لا استنتاج آلي مُخمَّن لصيغ تاريخية غير معروفة سلفاً**) · كل الدوامينات الخمسة مُهاجَرة بنجاح الخطوات 1-6 بـ§6 · Characterization Diff نظيف للجميع · Full Regression (نفس 30 ملفاً أو أكثر) + Fuzz 200/200 خضراء بنفس الأرقام · اختبارات الـBoundary الجديدة (§8-ج) خضراء · نتيجة Audit (0,0) موثَّقة والقرار المترتب عليها (سواء تنفيذ CHECK جديد أو تأجيله) موثَّق بوضوح · `returns.py` لم يُلمَس (يبقى خارج 3B-4 كما تقرَّر) · `invoice_no` لم يُلمَس · لا Regression بنتائج 3B-1/2/3 بأي رقم واحد.
